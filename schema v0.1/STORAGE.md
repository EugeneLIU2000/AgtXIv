...
目标顺序：**SQL 运行与候选记账 → 验证导出 → Git 封存字节和 manifest → Neo4j 按批次重建投影**。

Git 是版本封存工具及其仓库格式；GitHub 是托管 Git 仓库的远端位置。这里的封存由 Git 记录精确版本，远端保存默认使用独立、私有的 GitHub 数据仓库；二者都不授予科学认可。

直觉：SQL 是仍在工作的实验记录本，Git 是某次检查后的封存副本，Neo4j 是从副本绘制的关系地图。封存和画进地图都不表示实验结论正确。
...
| Git 封存层 | 经验证的一次固定快照及其清单 | 对已封存的精确字节负责；不能还原从未导出的 SQL 更新 |
| Neo4j | 从固定封存批次产生的可丢弃投影 | 无独立修改业务含义或批准复用的权限 |
...
1. 在同一个 SQL 一致性读截面固定精确 snapshot、记录/工件闭包、运行事件最高序号和 schema/policy 身份。导出任务通过同事务 outbox 创建；消息承载固定输入，禁止重试时改读最新头。
...
4. 在专用封存工作区创建 commit。以预期分支头比较更新；竞争时保留本次包，重新核验后追加，不强推、不覆盖旧封存。push 后读回约定远端的精确 commit 和 manifest 字节，再记录封存成功；本地 commit 成功与远端已持久保存是不同状态。
5. Neo4j 导入只读取已核验的封存批次。每个批次逻辑隔离，先 BUILDING，按固定次序幂等导入节点再导入关系；每个导入块提交后记录 checkpoint。重试须核对消息和批次哈希。完整性核验必须比较精确节点/边集合及哈希，不能只看总数。
6. 仅在闭包、前提、路线与集合校验通过后标记 READY，再以预期旧批次做可见头切换。图事务不能与 SQL/Git 跨系统原子提交；SQL checkpoint 落后于图时重放核对，不能凭 SQL 标志盲信图已完成。查询失败或批次未 READY 时返回未就绪，不偷偷选另一个快照。
...
每次查询至少返回：精确 snapshot_ref、schema_bundle_hash、SQL 读截面/事件边界、batch_id、manifest_sha256、Git commit、projection_version、view、投影 checkpoint、完整性状态、截断/缺失引用及权限过滤说明。直接 SQL 候选查询尚未封存时，Git/图字段明确为 null；图查询返回其导出时的 SQL 边界而不是当前数据库头。分页游标必须绑定同一边界和筛选条件。
...
恢复顺序：恢复 SQL 及工件并验证哈希/引用 → 核对远端封存回执 → 重放未确认 outbox → 从封存重新构建图 → 校验后切换图头。仅有 Git 时只能恢复已封存截面，必须报告丢失的未封存区间。Neo4j volume 可用于缩短恢复时间，但可重建性不代表 SQL 可丢弃。故障矩阵：SQL 已提交/Git 未写则重试；Git 已写/回执未写则按哈希认领原 commit；图部分写则保留 BUILDING 并幂等续传；图已完成/SQL checkpoint 未写则验证图后补记。备份恢复后必须提升执行代次或重新建立租约以拒绝旧 worker。
