...
\end{equation}
The appendix derives the polar rotation for $G=\mathrm{SO}(3)$ and states $\tfrac13\operatorname{Tr}([F_X])I$ for $G=\mathbb R_+^*I$. The source's scopes do not align: the main text invokes positive determinant, whereas the appendix calls the matrix positive definite. Directly minimizing $h(\lambda)=3\lambda^2-2\lambda\operatorname{Tr}(B)+\lVert B\rVert_F^2$ on the open domain $\lambda>0$ shows that the stated minimizer exists uniquely when $\operatorname{Tr}(B)>0$; positive definiteness suffices, but positive determinant alone does not. For $\operatorname{Tr}(B)\leq0$, the infimum lies at the excluded boundary $\lambda=0$, so the argmin is empty. The record preserves the paper's assertion and labels this condition check as reviewer assessment rather than silently repairing the source.
...
\begin{sloppypar}
The five-field accepted identity kernel---\field{claim\_id}, \field{revision}, \field{statement}, \field{source\_spans}, and \field{representation\_hash}---can preserve this paper without encoding full bundle theory. Candidate admission must still treat typed notation, scope, modality, and attribution as first-class semantics and project every truth-relevant value into the statement. Postulates and derived equivalences remain distinguishable through annotations and relations, and one named formal construction may remain composite when splitting would destroy its identity. A later typed relation layer for equivalence, induced maps, restrictions, and group actions is preferable to enlarging the accepted identity kernel.
\end{sloppypar}