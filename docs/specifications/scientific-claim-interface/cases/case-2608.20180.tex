...
The paper supports a small interface with a source-grounded scientific-claim envelope and an optional \texttt{MathClaimIR} payload. The envelope must retain exact spans, conditions, modality, attribution, and a typed relation such as ``sufficient construction,'' ``local equivalence,'' or ``limiting implication.'' A formula-only representation is too weak, but a sentence-level ontology is unnecessarily large. The 48 candidates show that repeated formulas can be consolidated aggressively if non-contiguous evidence and source-level qualifications remain first-class.