...
This paper again supports a compact \ScientificClaim{} core: statement, source spans, scope, modality, and attribution. It strongly argues against putting ``proof status'' into a single Boolean. The useful structure is relational: an assumption supports a derivation; a finite algorithm implements or approximates a dynamics; an observation agrees or conflicts with an expectation; and an assessment flags normalization, sign, or evidence-scope problems. \MathClaimIR{} should normalize the exact equations without certifying either their applicability to the simulation or their scientific correctness.