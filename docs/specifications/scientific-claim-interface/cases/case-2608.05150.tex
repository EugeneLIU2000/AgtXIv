...
headed ``Interface analysis'' report the present stress-test analysis. The machine-readable inventory contains 35 consolidated candidates and no deliberate composites. It is nevertheless candidate-stage stress data, and promotion to an accepted ScientificClaim still requires the later atomicity gate.
...
The five-field accepted identity kernel handles the paper's equation-level
content when its statement carries every truth-relevant condition and its
source spans are exact. Scope, modality, and attribution remain first-class
admission semantics even though they are not separate accepted-core columns.
The interface is only partially adequate for model-family conclusions and
numerical comparisons, and it must allow \textsc{no} normalization without
treating the scientific claim as invalid. Source fidelity, mathematical
normalization, and scientific or phenomenological acceptance remain separate.