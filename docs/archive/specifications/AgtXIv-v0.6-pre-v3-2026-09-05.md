> Historical design snapshot; non-normative. Archived before the V3 redesign on 2026-09-05.
> Original root-file SHA-256: `ecd256bd070fbd4b909364e12257864cf901ddc3ec8eb0c778377dcffc44b336`.
> Original repository HEAD: `4bc41cbd32a0bb6b5fa1ea2a0b69c972525b28bd`.
> The original bytes remain in that Git history; this reading copy adds this notice and relocates relative Markdown links only.
> Current design: [AgtXIv V3](../../../AgtXIv.md). V1 and V2 keep their own version-specific authority.
...
**Pending highest-level authority proposal:** [AgtXIv Charter](../../../CHARTER.md), `AgtXIv-Charter/1.0`
...
The proposed [AgtXIv Charter](../../../CHARTER.md) becomes the highest-level normative authority only upon the authorized ratification defined by ADR 0007. From that point, the authority hierarchy is Charter > version specifications and governance > ADRs, contracts, schemas, and plans > implementation. The [V2 paper-agentization specification](../../../docs/specifications/v2-paper-agentization.md) establishes the primary release-centered, query-independent architecture and is authoritative for V2 construction, completion, roles, releases, and query modes within the authority then applicable. In that namespace, the **V2 Root Agent is the accountable paper-level verification/audit authority**: it verifies the exact candidate package in dependency order, preserves unresolved states, and issues the release recommendation. The distinct **ReleaseCertifier is independent and mechanical**: it may certify only an exact `RECOMMEND_PROFILE_RELEASE` audit for which every required mechanical check passes; otherwise it records rejection. This binds the immutable Root Agent audit, hashes, policy, and release identity without self-certification. The current body of `AgtXIv.md` preserves V1 compatibility semantics for bounded query-relative `ClaimMathBridge` / `BridgeAssessment` workflows; it must not be read as making those workflows canonical V2 package construction. If ratified, Charter adoption does not rewrite, promote, reclassify, or invalidate immutable pre-adoption records, which remain interpreted under the exact contracts they bound. The paper-source acquisition specification governs canonical full-text acquisition, and the mathematics pipeline documents legacy/downstream mathematics workflows. Safety and non-promotion rules remain mandatory across versions.
